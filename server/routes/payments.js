...
router.post('/create-checkout-session', async (req,res)=>{
  const { productId, email } = req.body; const p = db.prepare('SELECT * FROM products WHERE id=?').get(productId);
  if(!p) return res.status(400).json({error:'Produit inconnu'});
  const session = await stripe.checkout.sessions.create({
    payment_method_types:['card'],
    line_items:[{price_data:{currency:p.currency,product_data:{name:p.title},unit_amount:p.price_cents},quantity:1}],
    mode:'payment',
    ...
    metadata: {productId: p.id, userEmail: email}
  });
...
router.post('/create-crypto-charge', async (req,res)=>{
  const { productId, email } = req.body; const p = db.prepare('SELECT * FROM products WHERE id=?').get(productId);
  if(!p) return res.status(400).json({error:'Produit inconnu'});
  const chargeData = {name:p.title,description:`Paiement ${p.title}`,local_price:{amount:(p.price_cents/100).toFixed(2),currency:p.currency.toUpperCase()},pricing_type:'fixed_price'};
...
// Stripe webhook: auto-deliver a key when checkout.session.completed
router.post('/webhook/stripe', express.raw({type:'application/json'}), (req,res)=>{
  const sig = req.headers['stripe-signature'];
  let event;
  try{
    event = stripe.webhooks.constructEvent(req.body, sig, process.env.STRIPE_WEBHOOK_SECRET);
  }catch(e){ console.error('stripe webhook error',e); return res.status(400).send(`Webhook Error: ${e.message}`); }
  if(event.type === 'checkout.session.completed'){
    ...
    if(ord){
      ...
      // Auto-generate delivery key and store
      const key = 'KEY-' + Math.random().toString(36).slice(2,12).toUpperCase();
      db.prepare('INSERT INTO delivered_keys(order_id,key_text) VALUES(?,?)').run(ord.id,key);
      console.log('Auto-delivered key for order', ord.id, key);
      // In real life: send email to ord.email with the key
    }
  }
  res.json({received:true});
});

// Coinbase webhook (simplified): auto-deliver on charge:confirmed
